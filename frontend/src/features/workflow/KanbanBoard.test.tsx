...
const mockOrders = [
  ...
  { id: '2', vehiclePlate: 'XYZ-9876', vehicleModel: 'Palio', description: 'Revisão', status: 2, isNfEmitted: false, createdAt: new Date().toISOString() }, // Aprovado
];
...
const server = setupServer(
  ...
  http.post('*/serviceorders', async () => {
    return HttpResponse.json({ id: '999', vehiclePlate: 'XYZ-1234', vehicleModel: 'Civic', description: 'Revisão', status: 0, isNfEmitted: false, createdAt: new Date().toISOString() }, { status: 201 });
  }),
  ...
  http.post('*/serviceorders/:id/emit-nf', async ({ params }) => {
    const { id } = params;
    return HttpResponse.json({ id, status: 4, isNfEmitted: true });
  })
);
