...
function DashboardLayout() {
        ...
        return (
          <div className="bg-white shadow-sm rounded-xl border border-gray-100 overflow-hidden">
            <div className="px-6 py-5 border-b border-gray-100 bg-white">
              ...
              <p className="text-sm text-gray-500 mt-1">Visualize e gerencie os dados dos seus clientes e seus veículos.</p>
            </div>
            <div className="p-6">
              <CustomerList />
            </div>
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col md:flex-row font-sans text-gray-900 overflow-hidden h-screen">
      
      {/* Sidebar Navigation */}
      {isSidebarOpen && (
      <aside className="w-full md:w-64 bg-white border-r border-gray-200 flex flex-col shadow-sm z-10 md:min-h-screen relative text-primary shrink-0 h-full">
        <button 
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50/50 flex items-center justify-between gap-3 shrink-0">
          <div className="flex items-center gap-3 overflow-hidden">
            <div className="w-10 h-10 rounded-full bg-blue-100 flex-shrink-0 flex items-center justify-center text-primary font-bold shadow-inner text-sm uppercase">
              {user?.name?.substring(0, 2).toUpperCase() || 'AF'}
            </div>
            <div className="flex flex-col overflow-hidden text-left">
              <span className="text-sm font-semibold text-gray-800 truncate">{user?.name || 'Admin Oficina'}</span>
              <span className="text-[10px] text-gray-500 uppercase tracking-wide">{user?.role === 'Mechanic' ? 'Mecânico' : 'Administrador'}</span>
            </div>
          </div>
          <button 
        ...
        <header className="bg-white border-b border-gray-200 h-16 flex items-center justify-between px-4 md:px-8 shadow-sm shrink-0">
          ...
          <div className="flex items-center gap-4">
            {user?.role === 'Mechanic' && (
              <button 
                onClick={handleLogout}
                className="flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-red-600 transition-colors mr-2"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                </svg>
